...
const fmt = (n: number, ccy = 'USD', decimals = 0) =>
  new Intl.NumberFormat('en-US', { minimumFractionDigits: decimals, maximumFractionDigits: decimals }).format(n) + ' ' + ccy

const fmtDate = (d: string) => new Date(d).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })

// Current prices from the March 24 xlsx
...
function toUSD(amount: number, ccy: string) {
  return amount * (FX[ccy] ?? 1)
}
...
export default function Dashboard() {
  ...
  const unrealizedPL = totalValueUSD - totalCostUSD

  const ytdGross = received.reduce((sum, d) => sum + toUSD(d.gross_amount, d.currency), 0)
  const ytdNet = received.reduce((sum, d) => sum + toUSD(d.net_amount, d.currency), 0)
  ...
  const chartData = projections.slice(0, 8).map(p => ({
    symbol: p.symbol,
    amount: Math.round(toUSD(p.projected_total ?? 0, p.currency)),
  }))

  // Currency exposure
  const byCcy: Record<string, number> = {}
  holdings.forEach(h => {
    const v = toUSD((PRICES[h.symbol] ?? h.avg_price) * h.shares, h.currency)
      ...
      <main style={{ marginLeft: 220, flex: 1, padding: '32px 36px', maxWidth: 1100 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Symbol', 'Shares', 'Price', 'Mkt value', 'Unr. P&L', '2027 div'].map((h, i) => (
                ...
                <tbody>
                  {holdings.map(h => {
                    ...
                    const projTotal = proj ? toUSD(proj.projected_total ?? 0, proj.currency) : null
                    return (
                      <tr key={h.id} style={{ cursor: 'default' }}>
                        ...
                        <td style={{ padding: '9px 14px', borderBottom: '1px solid var(--border)', textAlign: 'right', color: 'var(--text2)' }}>
                          {mktVal.toLocaleString(undefined, { maximumFractionDigits: 0 })} <span style={{ fontSize: 10, color: 'var(--text3)' }}>{h.currency}</span>
                        </td>
                        <td style={{ padding: '9px 14px', borderBottom: '1px solid var(--border)', textAlign: 'right', color: pl >= 0 ? 'var(--green)' : 'var(--red)' }}>
                          {pl >= 0 ? '+' : ''}{pl.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                        </td>
                        <td style={{ padding: '9px 14px', borderBottom: '1px solid var(--border)', textAlign: 'right', color: projTotal ? 'var(--green)' : 'var(--text3)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  return (
                    <div key={ccy}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text2)', marginBottom: 4 }}>
                        <span>{ccy}</span><span style={{ color: colors[ccy] ?? 'var(--text3)' }}>{pct}%</span>
                      </div>
            ...
            <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', flex: 1 }}>
              <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text2)' }}>2027 income by holding</span>
              </div>
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Badge variant="green">YTD gross: ${ytdGross.toFixed(2)}</Badge>
          </div>
            ...
            {received.map(d => (
              <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ color: 'var(--green)' }}>+{fmt(d.gross_amount, d.currency, 2)}</div>
                  <div style={{ fontSize: 10, color: 'var(--text3)' }}>WHT −{fmt(d.withholding_tax, d.currency, 2)}</div>
                </div>
              </div>
            ))}
          </div>
